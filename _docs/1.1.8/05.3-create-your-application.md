...
>**🚩 Note:** Since Averos does not support other styling types, you should specify the `scss` styling type when creating a new angular application. Other styling types are likely to be supported in future releases.
{: .notice--danger}
...
The averos integration wizard, triggered via `ng add @wiforge/averos`, will take you through the following customization parameters:

 - [x] your application name 
 - [x] the authentication backend url. please refer to [setting up your averos authentication factory](#iv--authentication-provider "Authentication Provider SetUp")  section.
