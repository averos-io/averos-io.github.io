...
```bash
ng add @averos/workflow:averos-entity --name=MyEntity [--sname=MyEntityService]
```
...
### Command Usage
```bash
ng g @averos/workflow:add-simple-member --ename=MyEntity --mname=isActive --member-type=Boolean [--list-of-enum-values=value1,value2,value3]
```
### Input Parameters
