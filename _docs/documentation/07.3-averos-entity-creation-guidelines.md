...
export class ToDoArea {
    ...
    toDoTasks: ToDoTask[];
    @OneToOne('User', import('@averos/workflow/core/_models/user')) createdBy?: User;
    @OneToOne('User', import('@averos/workflow/core/_models/user')) updatedBy?: User;
...
>🙋‍♂️ Though averos entity members, could be added manually, we recommend using the averos workflow `ng g @averos/workflow:add-simple-member` for adding simple typed members and `ng g @averos/workflow:add-composite-member` for adding composite members to the averos business entity.
