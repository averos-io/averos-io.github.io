...
One particular custom application initializer, will be created by the workflow command `ng add @averos/workflow`, and which will be in charge of registering any custom business entity into the averos framework context in order to make their entities metadata available to the framework beforehand.
