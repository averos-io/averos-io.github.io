...
>ℹ️ **Example:**
```shell
ng add @averos/workflow --application-name=MyAverosApplication --enable-authentication --authentication-provider=custom --default-language-code=en
```
...
The user may use `ng add @averos/workflow` interactively. In this case the workflow will guide the user through all of the available parameters.
{: .notice--success}

>🔖 Note that, the averos workflow  `ng add @averos/workflow` is considered as your starting point and will include the averos core module into your application, automatically activating all the averos aspects mentioned above.
{: .notice--info}
